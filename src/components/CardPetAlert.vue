<template>
  ...
  <div class="container" v-if="isContentLoaded">
    <section class="title">
      <h1 class="title__page" v-if="cardList.alert_type == 'Perdu'">
        ALerte : Perdu {{ cardList.meta.petSpecies }} nommé
        {{ cardList.meta.petName }}
      </h1>
      <h1 class="title__page" v-if="cardList.alert_type == 'Trouvé'">
        ALerte : Trouvé {{ cardList.meta.petSpecies }}
        <span v-if="cardList.meta.petName"
          >nommé {{ cardList.meta.petName }}</span
        >
      </h1>
      <h1 class="title__page" v-if="cardList.alert_type == 'Kidnapé'">
        ALerte : Kidnapé {{ cardList.meta.petSpecies }} nommé
        {{ cardList.meta.petName }}
      </h1>
    </section>
    ...
    <section class="box">
      ...
      <article>
        ...
        <div class="cardAnimal">
            ...
            <div class="cardAnimal__lost__status">
              <p>
                <span
                  ...
                  v-if="cardList.alert_type == 'Perdu'"
                  >Perdu</span
                >
                <span
                  class="cardAnimal__lost__lostAnimalStatusBlue"
                  v-if="cardList.alert_type == 'Trouvé'"
                  >Trouvé</span
                >
                <span
                  class="cardAnimal__lost__lostAnimalStatusBlue"
                  v-if="cardList.alert_type == 'Kidnapé'"
                  >Kidnapé</span
                >
              </p>
            </div>
          ...
          <div
            ...
            v-html="cardList.content.rendered"
          ></div>
        </div>
      </article>
      ...
      <button
        v-if="cardList.meta.contactMail"
        class="blueButton"
        @click="contactOwner(cardList.meta.contactMail)"
      >
        <i class="fa-solid fa-envelope"></i>Contacter le propriétaire
      </button>
      <button
        v-if="cardList.meta.contactPhone"
        class="blueButton"
        @click="phoneToOwner(cardList.meta.contactPhone)"
      >
...
export default {
  ...
  mounted() {
    const singlePet =
      baseUrl + "/wp/v2/alert/" + this.$route.params.alertId + "?_embed";
